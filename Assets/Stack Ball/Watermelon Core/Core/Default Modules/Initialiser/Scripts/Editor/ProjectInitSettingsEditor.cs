...
namespace Watermelon
{
    ...
    public class ProjectInitSettingsEditor : WatermelonEditor
    {
        ...
        protected override void OnEnable()
        {
            ...
            //Load all modules
            allowedTypes = Assembly.GetAssembly(typeof(InitModule)).GetTypes().Where(type => type.IsClass && !type.IsAbstract && (type.IsSubclassOf(typeof(InitModule)) || type.Equals(typeof(InitModule)))).ToArray();
            ...
            for (int i = 0; i < allowedTypes.Length; i++)
            {
                ...
                modulesGenericMenu.AddItem(new GUIContent(Regex.Replace(tempType.Name.Replace("Manager", " Manager"), "([a-z]) ?([A-Z])", "$1 $2").Replace("Init Module", "")), false, delegate
                {
                    AddModule(tempType);
                });
            }
